...
class DataArguments:
    ...
    dataset_files: Optional[List[str]] = field(
        default=None,
        metadata={"help": "Optional list of dataset files. When provided, must correspond 1:1 with lang_pairs_order."},
    )
    ...
    lang_pairs_order: Optional[List[str]] = field(
        default=None,
        metadata={"help": "Optional list of language-pairs mapping to dataset_files by index, e.g. ['zh-en','zh-my', ...]"},
    )

    target_lang_pair: Optional[str] = field(
        default=None,
        metadata={"help": "If set, treat loaded dataset as examples of this language-pair (canonical form 'zh-en')."},
    )

    current_forced_lang_pair: Optional[str] = field(
        default=None,
        metadata={"help": "(runtime) current language-pair to force on loaded examples (set by training loop)."},
    )
    ...
    def __post_init__(self):
        ...
        self.dataset_files = split_arg(self.dataset_files)
        self.lang_pairs_order = split_arg(self.lang_pairs_order)
        ...
        if self.dataset_files is not None and self.lang_pairs_order is not None:
            if len(self.dataset_files) != len(self.lang_pairs_order):
                raise ValueError("`dataset_files` and `lang_pairs_order` must have identical lengths when both are provided.")
