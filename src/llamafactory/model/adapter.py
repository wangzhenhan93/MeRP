...
def _is_prompt_encoder_param(name: str) -> bool:

    try:
        return "prompt_enc_" in str(name)
    except Exception:
        return False
...
def _setup_full_tuning(
    ...
    for name, param in model.named_parameters():
        if _is_prompt_encoder_param(name):
            try:
...
def _setup_lora_tuning(
    ...
    if model_args.adapter_name_or_path is not None:
        ...
        for adapter in adapter_to_merge:
            if adapter in loaded_adapters:
                logger.info_rank0(f"Adapter {adapter} is already loaded, skipping.")
                continue  
        ...
        if adapter_to_resume is not None:  
            ...
            loaded_adapters.add(adapter_to_resume)  
    ...
    try:
        prompt_enc_params = [n for n, _ in model.named_parameters() if _is_prompt_encoder_param(n)]
        if len(prompt_enc_params) > 0:
            logger.info_rank0(
                f"Detected prompt encoder parameters after adapter setup: count={len(prompt_enc_params)} example_names={prompt_enc_params[:6]}"
            )
    except Exception:
        pass
