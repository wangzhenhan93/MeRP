...
def patch_config(
    ...
    _archs = getattr(config, "architectures", None)
    if _archs is None:
        _archs = []
    elif isinstance(_archs, str):
        _archs = [_archs]
    else:
        try:
            iter(_archs)
        except TypeError:
            logger.info_rank0(f"[patch_config] Unexpected config.architectures type: {type(getattr(config, 'architectures'))}, coercing to [].")
            _archs = []

    logger.info_rank0(f"[patch_config] architectures = {_archs}")
    if "InternVLChatModel" in _archs:
        raise ValueError(
    ...
    if "LlavaLlamaForCausalLM" in _archs:
