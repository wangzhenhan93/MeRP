...
def create_custom_optimizer(
    ...
    # 原有逻辑，得到 base optimizer
    if finetuning_args.use_galore:
        optimizer = _create_galore_optimizer(model, training_args, finetuning_args)
    elif finetuning_args.use_apollo:
        optimizer = _create_apollo_optimizer(model, training_args, finetuning_args)
    elif finetuning_args.loraplus_lr_ratio is not None:
        optimizer = _create_loraplus_optimizer(model, training_args, finetuning_args)
    elif finetuning_args.use_badam:
        optimizer = _create_badam_optimizer(model, training_args, finetuning_args)
    elif finetuning_args.use_adam_mini:
        optimizer = _create_adam_mini_optimizer(model, training_args)
    elif finetuning_args.use_muon:
        optimizer = _create_muon_optimizer(model, training_args)
    else:
        # fallback 默认 AdamW
        optimizer = torch.optim.AdamW(model.parameters(), lr=training_args.learning_rate)

    # --------- Add PromptManager params if enabled ----------
    if getattr(finetuning_args, "use_prompt_manager", False):
        prompt_params = []
        if hasattr(model, "prompt_manager"):
            for n, p in model.prompt_manager.named_parameters():
                if p.requires_grad:
                    prompt_params.append(p)
        if prompt_params:
            # 新建一个 param group，单独 lr 可用 finetuning_args.prompt_lr
            lr = getattr(finetuning_args, "prompt_lr", training_args.learning_rate)
            param_group = {"params": prompt_params, "lr": lr}
            if hasattr(optimizer, "param_groups"):
                optimizer.param_groups.append(param_group)
            else:
                # fallback: 重新创建 AdamW
                optimizer = torch.optim.AdamW(list(optimizer.param_groups[0]["params"]) + prompt_params, lr=lr)
