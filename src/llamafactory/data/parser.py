...
class DatasetAttr:
    ...
    lang_pair: Optional[str] = "lang_pair"
    ...
    def join(self, attr: dict[str, Any]) -> None:
        ...
        if "columns" in attr:
            column_names = ["prompt", "query", "response", "lang_pair", "history", "messages", "system", "tools"]
...
def normalize_lang_pair(raw: Optional[str]) -> str:
    """
    Normalize various representations of language-pair into canonical form 'src-tgt', e.g. 'zh-en'.
    Accepts: 'zh-en', 'zh_en', 'zh->en', 'zh2en', 'zh en', 'ZH-EN', etc.
    Returns lowercase canonical form or empty string if cannot parse.
    """
    if not raw:
        return ""
    ...
    s = re.sub(r'[\s_:]+', '-', s)
    s = re.sub(r'\s+', '-', s)
    m = re.match(r'^([a-z]{2,3})\s*2\s*([a-z]{2,3})$', s)
    if m:
