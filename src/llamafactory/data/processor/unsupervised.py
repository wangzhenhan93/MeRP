...
class UnsupervisedDatasetProcessor(DatasetProcessor):
    ...
    def preprocess_dataset(self, examples: dict[str, list[Any]]) -> dict[str, list[Any]]:
        ...
        for i in range(len(examples["_prompt"])):
            ...
            try:
                lp_val = examples.get("_lang_pair", [None] * len(examples["_prompt"]))[i]
                if lp_val is None:
                    lp_val = examples.get("lang_pair", [None] * len(examples["_prompt"]))[i]
                model_inputs["lang_pair"].append(str(lp_val) if lp_val is not None else "")
            except Exception:
                model_inputs["lang_pair"].append("")
